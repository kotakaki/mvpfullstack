...
## 6. Business Process Flow (Elite Point B)
- **Bước 1 (Intake)**: Người dùng nhập yêu cầu cơ bản (Điểm đến, Thời gian, Ngân sách, Sở thích).
- **Bước 2 (Research & Compose)**: Agent tìm kiếm dữ liệu qua các API (Chuyến bay, Khách sạn, Điểm đến) và tổng hợp thành 1-3 phương án.
- **Bước 3 (Review & Refine)**: Người dùng xem xét, yêu cầu điều chỉnh (nếu có). Agent cập nhật lại kế hoạch theo thời gian thực.
- **Bước 4 (Approval & Booking)**: Người dùng chốt phương án và thanh toán. Agent gọi API đặt chỗ và xuất vé/mã xác nhận.
- **Bước 5 (Post-Booking & Support)**: Gửi lịch trình hoàn chỉnh qua email/app và trực sẵn sàng cho các thay đổi đột xuất trong chuyến đi.
