...
- Soạn thảo `Design_Specs.md` (theo mẫu `Design_Specs_template.md`) bao gồm:
  1. Cấu trúc Atomic (Tokens, Colors, Typography).
  2. **Screen Inventory**: Danh sách cụ thể các màn hình cần thiết kế để đáp ứng toàn bộ User Flow (từ điểm chạm đầu tiên đến khi kết thúc/lỗi). Mỗi màn hình cần có mô tả ngắn gọn chức năng.
  3. Link prototype Stitch và Hướng dẫn cho Developer.
